...
namespace mRemoteNG.UI.Menu
{
    ...
    {
        private ToolStripMenuItem _mMenViewTileConnections;
        ...
        private void Initialize()
        {
            ...
            _mMenViewTileConnections = new ToolStripMenuItem();
            ...
            // 
            // mMenViewTileConnections
            // 
            _mMenViewTileConnections.Image = Resources.Panels;
            _mMenViewTileConnections.Name = "mMenViewTileConnections";
            _mMenViewTileConnections.Size = new System.Drawing.Size(228, 22);
            _mMenViewTileConnections.Text = "Organizar todas (Grid View)";
            _mMenViewTileConnections.Click += mMenViewTileConnections_Click;
            // 
        ...
        private void mMenViewTileConnections_Click(object sender, EventArgs e)
        {
            ...
            foreach (var win in connections)
            {
                foreach (Crownwood.Magic.Controls.TabPage tab in win.TabController.TabPages)
                {
                    allTabs.Add(tab);
                }
                win.TabController.TabPages.Clear();
            }

            if (allTabs.Count == 0) return;

            int cols = (int)Math.Ceiling(Math.Sqrt(allTabs.Count));
            int rows = (int)Math.Ceiling((double)allTabs.Count / cols);

            ConnectionWindow[,] grid = new ConnectionWindow[cols, rows];
            ...
            for (int y = 0; y < rows; y++)
            {
                ...
                {
                    if (tabIndex >= allTabs.Count) break;

                    ConnectionWindow win;
                    if (tabIndex < connections.Count) {
                        win = connections[tabIndex];
                    } else {
                        win = _panelAdder.AddPanel("", false) as ConnectionWindow;
                    }
            ...
            for (int i = tabIndex; i < connections.Count; i++) {
                connections[i].Close();
            }
            ...
            for (int y = 0; y < rows; y++)
            {
                for (int x = 0; x < cols; x++)
                {
                    ...
                    if (x == 0 && y == 0)
                    {
                        win.Show(FrmMain.Default.pnlDock, DockState.Document);
                    }
                    else if (x > 0 && y == 0)
                    {
                        double proportion = (double)(cols - x) / (cols - x + 1);
                        win.Show(grid[x - 1, 0].Pane, DockAlignment.Right, proportion);
                    }
                    else if (y > 0)
                    {
                        double proportion = (double)(rows - y) / (rows - y + 1);
                        win.Show(grid[x, y - 1].Pane, DockAlignment.Bottom, proportion);
                    }
                }
            }
        }
